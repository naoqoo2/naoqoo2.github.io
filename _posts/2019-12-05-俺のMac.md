...
クリップボード効率化のためにライセンス購入。  
画像の履歴も保持できたり、スニペッドも定義できるので便利。  
...
Slackで2本指スワイプに履歴移動を割り当てる。  

- 2本指で右にスワイプ → `command` + `←`
- 2本指で左にスワイプ → `command` + `→`

![image.png](/assets/images/20191205/image-2.png)

※メモ：スワイプの感度が悪く、設定が効いていないと勘違いしたため `0.22 → 0.15` に下げた。  

![image.png](/assets/images/20191205/image-4.png)

### Chromeで右側のタブを全部閉じる

Chrome標準では「右側のタブを閉じる」にショートカットキーがない。  
なので、Chrome拡張でショートカットを追加してからBetterTouchToolで呼び出している。  

- 3本指固定のTipTap左 → `option` + `shift` + `R`

![image.png](/assets/images/20191205/image-3.png)
...
Chrome標準ではショートカットキーがないので拡張機能を入れる。  
